...
const signupAction = (prevFormState, formData) => {
  ...
  if (!isNotEmpty(password) || (!hasMinLength(password, 6))) {
    error.push('You must provide password with at least 6 characters')
  }
  if (!isEqualToOtherValue(password, confirmPassword)) {
    error.push('Check password does not match entered password')
  }
  if (!isNotEmpty(firstName) || !isNotEmpty(lastName)) {
    error.push('Please provide your first and last names')
  }
  if (!isNotEmpty(role)) {
  ...
  if (acquisitionChannel.length < 1) {
    error.push('Please select at least one aquisition channel')
  }
...
export default function Signup() {
  ...
  const [formState, formAction, pending] = useActionState(signupAction, initialState)
